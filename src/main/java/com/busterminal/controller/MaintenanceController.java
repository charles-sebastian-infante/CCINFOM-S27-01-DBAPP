    ...
    private void showEditMaintenanceTypeForm(HttpServletRequest request, 
     ...
     private void createMaintenance(HttpServletRequest request, 
        ...
        try {
            ...
            // 2. VALIDATE STATUS (Available or Scheduled only)
            if (!"Available".equals(busStatus) && !"Scheduled".equals(busStatus)) {
                conn.rollback();
                request.setAttribute("error", 
                    "Maintenance can only be performed on buses with 'Available' or 'Scheduled' status. " +
                    "Current status: " + busStatus);
                showNewMaintenanceForm(request, response);
                return;
            }
            
            // 3. IF BUS IS SCHEDULED, HANDLE CASCADE CANCELLATION
            if ("Scheduled".equals(busStatus)) {
                // 3a. Find all future schedules for this bus
                ...
                // 3b. For each schedule, delete tickets
                ...
                // 3c. Cancel all future schedules
                for (Integer scheduleID : scheduleIDs) {
                ...
                // 3d. Clear staff assignments from this bus
            ...
            // 5. UPDATE BUS STATUS TO 'Under Maintenance'
            PreparedStatement updateBusStmt = conn.prepareStatement(
                "UPDATE Bus SET status = 'Under Maintenance' WHERE bus_id = ?");
            ...
            if ("Scheduled".equals(busStatus)) {
                request.setAttribute("success", 
                    ...
                    "Note: Bus was scheduled, so all future schedules have been cancelled " +
                    "and tickets have been refunded.");
            } else {
                request.setAttribute("success", "Maintenance created successfully");
            }
